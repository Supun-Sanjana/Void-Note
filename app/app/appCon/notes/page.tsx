...
const Note = () => {
  ...
  const [noteTitle, setNoteTitle] = useState<
    { Note_Id: number; Title: string; Content: string }[]
  >([]);
  const [loadNote, setLoadNote] = useState(false);
  const [displayNote, setDiplayNote] = useState<null | {
    Note_Id: number;
    Title: string;
    Content: string;
  }>(null);
  ...
  const handleNoteClick = (note: {
    ...
    setDiplayNote(note);
    setIsOpen(false); // 👈 hide the welcome message
  };
  ...
  return (
    <>
      <div className="flex h-screen">
        {/* Fixed Sidebar Menu */}
        <div className="fixed left-0 top-15 h-full w-[300px] z-10">
          <div className="rounded-[5px] p-2 mr-10 menu w-[280px] h-full bg-[#141414] flex flex-col">
            <div className="border-b pb-4 border-[#3A3D47] mb-5">
              <div
                className="flex gap-3 p-3 bg-black rounded-[10px] mt-5 cursor-pointer hover:bg-gray-900 transition-colors"
              ...
              <div className="space-y-3 pb-8 px-2">
                ...
                {noteTitle.length > 0 ? (
                  noteTitle.map((note, index) => (
                    <div
                      key={index}
                      className="text-white hover:text-blue-400 cursor-pointer group"
                    >
                      <span
                      ...
                      >
                        ...
                        <Trash2
                          size={20}
                          className="ml-2 opacity-0 text-red-300 group-hover:opacity-100 transition-opacity duration-200"
                          onClick={() => {
        ...
        {/* Main Content Area */}
        <div className="flex w-full mt-0 ml-[260px] bg-black min-h-screen">
  {/* Main Content Area */}
    ...
    {isOpen ? (
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-gray-400">
          <h2 className="text-xl mb-4 sticky top-0 bg-black/90 p-4 z-10 border-b border-gray-700">
            {displayNote.Title}
          </h2>
          <p className="p-4">{displayNote.Content}</p>
        </div>
      </div>
    ) : (
      // Welcome Message
